...
namespace ClickIt.UI.Settings.Panels
{
    internal sealed class AltarSettingsPanelRenderer(ClickItSettings settings)
    {
        ...
        private void DrawAltarWeightOverridesSection()
        {
            ...
            SettingsUiRenderHelpers.DrawToggleNodeControl("Valuable Upside", _settings.ValuableUpside, "When enabled, automatically chooses the altar option with modifiers that have weights above the threshold, even if the overall weight calculation would suggest otherwise.");
            SettingsUiRenderHelpers.DrawRangeNodeControl("Valuable Upside Threshold", _settings.ValuableUpsideThreshold, 1, 100, "Minimum weight threshold for upside modifiers to trigger the high value override. Modifiers with weights at or above this value will cause the plugin to choose that altar option.");
            SettingsUiRenderHelpers.DrawToggleNodeControl("Unvaluable Upside", _settings.UnvaluableUpside, "When enabled, automatically chooses the opposite altar option when modifiers have weights at or below the threshold, avoiding potentially undesirable choices.");
            SettingsUiRenderHelpers.DrawRangeNodeControl("Unvaluable Upside Threshold", _settings.UnvaluableUpsideThreshold, 1, 100, "Weight threshold that triggers the low value override. When any modifier has a weight at or below this value, the plugin will choose the opposite altar option.");
            SettingsUiRenderHelpers.DrawToggleNodeControl("Dangerous Downside", _settings.DangerousDownside, "When enabled, automatically avoids altar options with dangerous downside modifiers that have weights above the threshold.");
            SettingsUiRenderHelpers.DrawRangeNodeControl("Dangerous Downside Threshold", _settings.DangerousDownsideThreshold, 1, 100, "Maximum weight threshold for downside modifiers to trigger the dangerous override. Modifiers with weights at or above this value will cause the plugin to choose the opposite altar option.");
            SettingsUiRenderHelpers.DrawToggleNodeControl("Minimum Weight", _settings.MinWeightThresholdEnabled, "When enabled, the plugin will enforce a minimum final weight for altar options. If an option's final weight is below this value the plugin will avoid picking it (and will choose the opposite option if available).");
            SettingsUiRenderHelpers.DrawRangeNodeControl("Minimum Weight Threshold", _settings.MinWeightThreshold, 1, 100, "Minimum final weight (1 - 100) an option must have to be considered valid. If both options are below this value, neither will be auto-chosen.");
        ...
        private void DrawAlertSoundSection()
        {
            ...
            {
                SettingsUiRenderHelpers.TriggerButtonNode(_settings.OpenConfigDirectory);
            }
            SettingsUiRenderHelpers.DrawInlineTooltip("Open the plugin config directory where you should put 'alert.wav'");

            if (ImGui.Button("Reload Alert Sound"))
            {
                SettingsUiRenderHelpers.TriggerButtonNode(_settings.ReloadAlertSound);
            }
            SettingsUiRenderHelpers.DrawInlineTooltip("Reloads the 'alert.wav' sound file from the config directory");
        ...
        private void DrawUpsideModsSection()
        {
            ...
            SettingsUiRenderHelpers.DrawInlineTooltip("Set weights for upside modifiers. Higher values are more desirable and can influence recommended altar choices.");
            if (!isOpen)
                return;
            ...
            ImGui.Spacing();
            ImGui.Spacing();
            SettingsUiRenderHelpers.DrawSearchBar("##UpsideSearch", "Clear##UpsideClear", ref _settings.UiState.UpsideSearchFilter);
            ImGui.Spacing();
            DrawUpsideModsTable();
            ImGui.TreePop();
        }
        ...
        private void DrawDownsideModsSection()
        {
            ...
            SettingsUiRenderHelpers.DrawInlineTooltip("Set weights for downside modifiers. Higher values are more dangerous and can influence recommended altar choices.");
            if (!isOpen)
                return;
            ...
            ImGui.Spacing();
            ImGui.Spacing();
            SettingsUiRenderHelpers.DrawSearchBar("##DownsideSearch", "Clear##DownsideClear", ref _settings.UiState.DownsideSearchFilter);
            ImGui.Spacing();
        ...
        private void DrawUpsideModRow(string id, string name, string type)
        {
            ...
            if (_settings.ModAlerts != null)
            {
                ...
                bool currentAlert = _settings.GetModAlert(id, type);
                if (ImGui.Checkbox("##alert", ref currentAlert))
                {
                    _settings.ModAlerts[ClickItSettings.BuildCompositeKey(type, id)] = currentAlert;
                }
            }
        ...
        private void DrawModWeightSliderCell(string id, string type)
        {
            ...
            int currentValue = _settings.GetModTier(id, type);
            if (ImGui.SliderInt("##weight", ref currentValue, 1, 100))
            {
                _settings.ModTiers[ClickItSettings.BuildCompositeKey(type, id)] = currentValue;
            }
        }
        ...
        private static string GetUpsideSectionHeader(string type)
            => type switch
            {
                ClickItSettings.AltarTypeMinion => "Minion Drops",
                ClickItSettings.AltarTypeBoss => "Boss Drops",
                ClickItSettings.AltarTypePlayer => "Player Bonuses",
                _ => string.Empty
            };
        ...
        private static void DrawDownsideSectionHeaderIfNeeded(ref string lastProcessedSection, string sectionHeader)
        {
            ...
            DrawSectionHeaderRow(sectionHeader, GetDownsideSectionHeaderColor(sectionHeader), new Vector4(1f, 1f, 1f, 1f));
        }
        ...
        private static Vector4 GetUpsideSectionHeaderColor(string type)
            => type switch
            {
                ClickItSettings.AltarTypeMinion => new Vector4(0.2f, 0.6f, 0.2f, 0.3f),
                ClickItSettings.AltarTypeBoss => new Vector4(0.6f, 0.2f, 0.2f, 0.3f),
                ClickItSettings.AltarTypePlayer => new Vector4(0.2f, 0.2f, 0.6f, 0.3f),
        ...
        private static Vector4 GetUpsideModTextColor(string type)
            => type switch
            {
                ClickItSettings.AltarTypeMinion => new Vector4(0.4f, 0.8f, 0.4f, 1.0f),
                ClickItSettings.AltarTypeBoss => new Vector4(0.8f, 0.4f, 0.4f, 1.0f),
                ClickItSettings.AltarTypePlayer => new Vector4(0.4f, 0.7f, 0.9f, 1.0f),
