...
// Blight's Recent Stages buffer — backed by the shared dedup buffer so it stays byte-for-byte compatible with pathfinding's Recent Events (same timestamp + (xN) dedup + 128 cap).
