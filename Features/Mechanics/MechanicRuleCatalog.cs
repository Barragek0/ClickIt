namespace ClickIt.Features.Mechanics
{
    internal static class MechanicRuleCatalog
    {
        ...
        internal static bool TryResolveSettlersOreMechanicId(string? path, out string? mechanicId)
        {
            ...
            {
                (string MechanicId, string Marker, bool ExcludeVerisiumBossTransition) = SettlersOreRules[i];
                if (!MatchesSettlersOrePathMarker(path, Marker))
                    continue;

                if (ExcludeVerisiumBossTransition
