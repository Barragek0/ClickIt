...
namespace ClickIt.Tests
{
    ...
    public class MockClickItSettings
    {
        ...
        // Backwards-compatible aliases used by integration tests
        public Dictionary<string, int> ModTiers => _modWeights;
        public Dictionary<string, int> ModWeights => _modWeights;
        ...
        public void SetModWeight(string modId, int weight)
        {
            _modWeights.TryGetValue(modId, out var old);
            _modWeights[modId] = weight;
            _updateHandler?.OnUpdate(new MockSettingsUpdate { PropertyName = "ModWeights", OldValue = old, NewValue = weight });
        }
        ...
        public void SetClickDistance(int distance)
        {
            var old = ClickDistance;
            ClickDistance = distance;
            _updateHandler?.OnUpdate(new MockSettingsUpdate { PropertyName = "ClickDistance", OldValue = old, NewValue = distance });
        }
        ...
        public void SetDebugMode(bool enabled)
        {
            var old = DebugMode;
            DebugMode = enabled;
            _updateHandler?.OnUpdate(new MockSettingsUpdate { PropertyName = "DebugMode", OldValue = old, NewValue = enabled });
        }
    ...
    // MockAltarComponent and MockSecondaryAltarComponent moved to Shared/MockAltarComponent.cs
