...
namespace ClickIt
{
    public class ClickItSettings : ISettings
    {
        ...
        [Menu("Enable Lazy Mode (not recommended, make sure you read the tooltip for this setting before enabling it) ->", "Will automatically click most things for you, without you needing to hold the key.\n\nThere are inherent limitations to this feature that cannot be fixed:\n\n-> If you are holding down a skill, for instance, Cyclone, you cannot interact with most things in the game.\n   If you use a skill that requires you to hold a key, you must set it to right-click (left-click in left-handed mode)\n   for lazy mode to function correctly.\n\n-> This will take control away from you at crucial moments, potentially causing you to die.\n\n-> The plugin cannot detect when a chest becomes unlocked, or if a settlers tree\n   has been activated. This is a limitation with exileapi and not the plugin and for this reason, Lazy Mode\n   is not allowed to click chests that were locked when spawned or the settlers tree. When one of these is on-screen,\n   Lazy Mode will be temporarily disabled, until the blacklisted item is off of the screen, which will\n   allow you to manually press the hotkey to click these items specifically if you want to.", 1, 3050)]
        ...
        [Menu("Lazy Mode Disable Hotkey", "When Lazy Mode is enabled and active, holding this key will temporarily disable lazy mode clicking.\nThis allows you to pause automated clicking without disabling lazy mode entirely.\nThe main hotkey will still work for manual clicking and other functions.", 3, 3050)]
        public HotkeyNode LazyModeDisableKey { get; set; } = new HotkeyNode(Keys.F2);
        [Menu("Restore cursor position after each click in lazy mode", "When enabled, restores cursor to original position after clicking in lazy mode.", 4, 3050)]
        public ToggleNode RestoreCursorInLazyMode { get; set; } = new ToggleNode(true);
        [Menu("Lazy Mode UIHover Sleep (ms)", "Sleep duration before UIHover verification in lazy mode. Increase if you notice the mouse moving and not successfully clicking on things when it should.\n\nA value of 20 is recommended.", 5, 3050)]
