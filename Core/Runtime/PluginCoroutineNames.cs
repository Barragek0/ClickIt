...
// The plugin's coroutine names, without a "ClickIt." prefix — ExileCore's runner already groups
// coroutines under the owning plugin, so the prefix is redundant in its coroutine list UI.
// Shutdown and hotkey re-attach look coroutines up by these exact names.
internal static class PluginCoroutineNames
{
    ...
    // scheduling by name, so sharing one name across overlays stops all but the first from running.
