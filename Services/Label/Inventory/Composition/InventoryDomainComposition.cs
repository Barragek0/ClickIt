...
{
    internal readonly record struct InventoryDomainCompositionDependencies(
        Func<Entity, string> GetWorldItemBaseName);

    internal static class InventoryDomainComposition
    {
        private const int InventoryProbeCacheWindowMs = 50;
        private const int InventoryDebugTrailCapacity = 32;

        internal static InventoryDomainFacade Create(InventoryDomainCompositionDependencies dependencies)
        {
            ...
            return new InventoryDomainFacade(
                dynamicAdapter,
                probeService,
                snapshotProvider,
                layoutParser,
