...
namespace ClickIt.Services
{
    ...
    {
        private sealed class MovementSkillCoordinator(ClickService owner)
        {
            public bool TryUseMovementSkillForOffscreenPathing(string targetPath, Vector2 targetScreen, bool builtPath, out Vector2 castPoint, out string debugReason)
            {
                ...
                int remainingNodes = owner.GetRemainingOffscreenPathNodeCount();
                int minimumNodes = Math.Max(1, owner.settings.OffscreenMovementSkillMinPathSubsectionLength?.Value ?? 8);
                long now = Environment.TickCount64;
                bool movementSkillsEnabled = owner.settings.UseMovementSkillsForOffscreenPathfinding?.Value == true;
                ...
                if (owner._lastMovementSkillUseTimestampMs > 0 && MovementSkillRecastDelayMs > 0)
                {
                    long elapsed = now - owner._lastMovementSkillUseTimestampMs;
                ...
                if (!ShouldAttemptMovementSkill(
                    ...
                    now,
                    lastSkillUseTimestampMs: owner._lastMovementSkillUseTimestampMs,
                ...
                if (!owner.EnsureCursorInsideGameWindowForClick("[TryUseMovementSkillForOffscreenPathing] Skipping cast - cursor outside PoE window"))
                {
                ...
                owner._lastMovementSkillUseTimestampMs = now;
                int postCastClickBlockMs = ResolveMovementSkillPostCastClickBlockMsForCast(movementSkillName);
                owner._movementSkillPostCastClickBlockUntilTimestampMs = postCastClickBlockMs > 0
                ...
                owner._movementSkillStatusPollUntilTimestampMs = statusPollWindowMs > 0
                    ? now + statusPollWindowMs
                    : 0;
                owner._lastUsedMovementSkillEntry = statusPollWindowMs > 0
                    ? movementSkillEntry
                    : null;
                owner.performanceMonitor.RecordClickInterval();
                owner.DebugLog(() => $"[TryUseMovementSkillForOffscreenPathing] Cast movement skill '{movementSkillName}' with key '{boundKey}'");
            ...
            public bool TryGetMovementSkillPostCastBlockState(long now, out string reason)
            {
                ...
                if (IsMovementSkillPostCastClickBlocked(now, owner._movementSkillPostCastClickBlockUntilTimestampMs, out long remainingMs))
                {
                    reason = $"timing window active ({remainingMs}ms remaining)";
                    return true;
                }

                if (owner._movementSkillStatusPollUntilTimestampMs <= 0 || now > owner._movementSkillStatusPollUntilTimestampMs)
                {
                    owner._movementSkillStatusPollUntilTimestampMs = 0;
                    owner._lastUsedMovementSkillEntry = null;
                    return false;
                }

                if (!TryResolveMovementSkillRuntimeState(owner._lastUsedMovementSkillEntry, out bool isUsing, out bool? allowedToCast, out bool? canBeUsed))
                    return false;
            ...
            public int ResolveMovementSkillPostCastClickBlockMsForCast(string? movementSkillInternalName)
            {
                ...
                return Math.Max(0, owner.settings.OffscreenShieldChargePostCastClickDelayMs?.Value ?? MovementSkillShieldChargePostCastClickBlockMs);
            }
            ...
            private bool TryGetSkillBarEntries(out IReadOnlyList<object?> entries)
            {
                ...
                object? skillBar = owner.gameController?.IngameState?.IngameUi?.SkillBar;
                if (skillBar == null)
                    return false;
